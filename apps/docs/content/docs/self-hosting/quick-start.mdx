...
Production with built-in Caddy and local source builds:

```bash
docker compose -f docker-compose.yml -f docker-compose.build.yml -f docker-compose.caddy.yml up -d --build
```
